...
//Projections --> way of getting required data only

// inclusion
//getting only name from data
db.movies.find({}, { name: 1 });  // 1 stands for inclusion 

//getting only name and rating from data 
db.movies.find({}, { name: 1, rating: 1 });  // 1 stands for inclusion 


// exclusion
//Filtering out summary from the data 
db.movies.find({}, { summary: 0 }).pretty();   // 0 stands for exclusion 

//SORTING

// asending sort --> "1" for asc sort 
db.movies.find({}).sort({ rating: 1 }).pretty();

// desending sort --> "-1" for desc sort 
db.movies.find({}).sort({ rating: -1 }).pretty();
db.movies.find({}, { _id: 0, name: 1, rating: 1 }).sort({ rating: -1 }).pretty();

// to limit the results upto required number --> use "limit(2)" --> 2 is the number of data user wants to limit 
db.movies.find({}, { _id: 0, name: 1, rating: 1 }).sort({ rating: -1 }).limit(2).pretty();

// to skip/offset data use "skip(2)" --> 2 is the number of data user wants to skip 
